...
func newItemsSetChildrenCheckboxCmd(app *App) *cobra.Command {
	var on bool
	var off bool

	cmd := &cobra.Command{
		Use:   "set-children-checkbox <item-id>",
		Short: "Set/unset checkbox rendering for direct children (owner-only)",
		Aliases: []string{
			"children-checkbox",
		},
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !on && !off {
				// Toggle by default.
				if strings.TrimSpace(t.ChildrenKind) == "checkbox" {
					want = ""
				} else {
					want = "checkbox"
				}
			...
			res, err := mutate.SetItemChildrenKind(db, actorID, id, want)
			if err != nil {
				return writeErr(cmd, err)
			}
			if !res.Changed {
			...
			t.UpdatedAt = time.Now().UTC()
			if err := s.AppendEvent(actorID, "item.set_children_kind", t.ID, res.EventPayload); err != nil {
				return writeErr(cmd, err)
			}
			if err := s.Save(db); err != nil {
				return writeErr(cmd, err)
	...
	cmd.Flags().BoolVar(&on, "on", false, "Enable checkbox rendering for direct children")
	cmd.Flags().BoolVar(&off, "off", false, "Disable checkbox rendering for direct children")
	return cmd
}

func newItemsSetItemKindCmd(app *App) *cobra.Command {
	var kind string

	cmd := &cobra.Command{
		Use:   "set-item-kind <item-id>",
		Short: "Set item kind override (owner-only)",
		Aliases: []string{
			"item-kind",
			"kind",
		},
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, err := mutate.SetItemKind(db, actorID, id, kind)
			if err != nil {
				return writeErr(cmd, err)
			}
			if !res.Changed {
			...
			t.UpdatedAt = time.Now().UTC()
			if err := s.AppendEvent(actorID, "item.set_item_kind", t.ID, res.EventPayload); err != nil {
				return writeErr(cmd, err)
			}
			if err := s.Save(db); err != nil {
				return writeErr(cmd, err)
	...
	cmd.Flags().StringVar(&kind, "kind", "", "Kind override: checkbox|status|inherit")
	return cmd
}
