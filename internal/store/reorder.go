...
func PlanReorderRanks(sibs []*model.Item, movedID string, insertAt int) (ReorderResult, error) {
	...
	// If the initial window can't be assigned due to "no space" (e.g. prefix-adjacent ranks like
	// "y" < "y0"), expand the window outward until we can assign or we reach a full rebalance.
	for steps := 0; steps < len(final)*2; steps++ {
		// Outer bounds.
		lower := ""
		upper := ""
		if lo > 0 {
		...
		if lower != "" && upper != "" && !(lower < upper) {
			// Keep expanding; if we can't find valid outer bounds, fall back to full rebalance.
			if lo == 0 && hi == len(final)-1 {
