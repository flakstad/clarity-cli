...
func newItemsCmd(app *App) *cobra.Command {
	...
	cmd.AddCommand(newItemsCopyCmd(app))
...
func newItemsCopyCmd(app *App) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			srcID := strings.TrimSpace(args[0])
			src, ok := db.FindItem(srcID)
			if !ok || src == nil {
				return writeErr(cmd, errNotFound("item", srcID))
			}

			destProjectID := strings.TrimSpace(projectID)
			if destProjectID == "" {
				destProjectID = strings.TrimSpace(src.ProjectID)
			}
			if _, ok := db.FindProject(destProjectID); !ok {
				return writeErr(cmd, errNotFound("project", destProjectID))
			}

			destOutlineID := strings.TrimSpace(outlineID)
			if destOutlineID == "" {
				// Default: keep outline if staying in the same project; otherwise follow create rules.
				if strings.TrimSpace(src.ProjectID) == destProjectID && strings.TrimSpace(src.OutlineID) != "" {
					destOutlineID = strings.TrimSpace(src.OutlineID)
				} else {
					var outlines []model.Outline
					for _, o := range db.Outlines {
						if o.ProjectID == destProjectID {
							outlines = append(outlines, o)
						}
					}
					if len(outlines) == 0 {
						destOutlineID = db.EnsureDefaultOutline(destProjectID, actorID, func(prefix string) string { return s.NextID(db, prefix) })
					} else if len(outlines) == 1 {
						destOutlineID = outlines[0].ID
					} else {
						return writeErr(cmd, errors.New("multiple outlines in project; pass --outline or create a new outline first"))
					}
				}
			} else {
				o, ok := db.FindOutline(destOutlineID)
				if !ok {
					return writeErr(cmd, errNotFound("outline", destOutlineID))
				}
				if o.ProjectID != destProjectID {
			...
			outline, ok := db.FindOutline(destOutlineID)
			if !ok || outline == nil {
				return writeErr(cmd, errNotFound("outline", destOutlineID))
			}

			// Default: keep parent only if we're copying within the same outline.
			...
			if strings.TrimSpace(parentID) != "" {
				...
				if parent != nil && strings.TrimSpace(parent.OutlineID) != destOutlineID {
					return writeErr(cmd, errors.New("parent must be in the same outline"))
				}
				tmp := parentID
				destParentID = &tmp
			} else if strings.TrimSpace(src.OutlineID) == destOutlineID && src.ParentID != nil && strings.TrimSpace(*src.ParentID) != "" {
				tmp := strings.TrimSpace(*src.ParentID)
				destParentID = &tmp
			}

			// Choose insertion rank.
			rank := nextSiblingRank(db, destOutlineID, destParentID)
			if strings.TrimSpace(src.OutlineID) == destOutlineID {
				sameParent := (src.ParentID == nil && destParentID == nil) ||
					(src.ParentID != nil && destParentID != nil && strings.TrimSpace(*src.ParentID) == strings.TrimSpace(*destParentID))
				if sameParent {
					sibs := []*model.Item{}
					for i := range db.Items {
						...
						if strings.TrimSpace(it.OutlineID) != destOutlineID {
							continue
						}
						if (it.ParentID == nil) != (destParentID == nil) {
							continue
						}
						if it.ParentID != nil && destParentID != nil && strings.TrimSpace(*it.ParentID) != strings.TrimSpace(*destParentID) {
							continue
						}
						sibs = append(sibs, it)
					}
					...
					if idx >= 0 {
						...
						upper := ""
						for j := idx + 1; j < len(sibs); j++ {
							r := strings.TrimSpace(sibs[j].Rank)
							if r != "" && lower != "" && r > lower {
								upper = r
								break
							}
			...
			// Default: if an agent copies an item, assign it to itself.
			var assigned *string
			...
			if err := s.AppendEvent(actorID, "item.create", it.ID, it); err != nil {
				return writeErr(cmd, err)
			}
			...
			hints := []string{
				"clarity items show " + it.ID,
				"clarity items move-outline " + it.ID + " --outline <outline-id>",
			}
			return writeOut(cmd, app, map[string]any{"data": it, "_hints": hints})
		},
	}

	cmd.Flags().StringVar(&projectID, "project", "", "Project id for the new item (default: source item project)")
	cmd.Flags().StringVar(&outlineID, "outline", "", "Outline id for the new item (default: source item outline when possible)")
	cmd.Flags().StringVar(&parentID, "parent", "", "Parent item id for the new item (default: source item parent when copying within same outline)")
