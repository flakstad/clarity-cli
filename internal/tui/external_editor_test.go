...
func TestApplyExternalViewEditorResult_CleansUp(t *testing.T) {
	t.Parallel()

	var m appModel

	dir := t.TempDir()
	path := filepath.Join(dir, "view.md")
	if err := os.WriteFile(path, []byte("body\n"), 0600); err != nil {
		t.Fatalf("write temp file: %v", err)
	}

	m.externalViewEditorPath = path
	m.applyExternalViewEditorResult(externalViewEditorDoneMsg{err: nil})
