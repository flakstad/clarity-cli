...
type App struct {
	...
	// Track explicit flag usage so we can distinguish env defaults (CLARITY_DIR, CLARITY_WORKSPACE)
	// from user intent on the command line.
	dirFlagSet       bool
...
`),
	...
	cmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		// Cobra treats env-provided persistent flag defaults (via envOr) the same as
		// user-provided flags. Capture which flags were *explicitly* set so we can
		// make `--workspace` override an ambient CLARITY_DIR default.
		app.dirFlagSet = cmd.Flags().Changed("dir")
		app.workspaceFlagSet = cmd.Flags().Changed("workspace")
		return nil
	}
...
func loadDB(app *App) (*store.DB, store.Store, error) {
	dir := app.Dir
	// If the user explicitly chose a workspace, treat an env-provided CLARITY_DIR
	// default as unset (unless --dir was explicitly provided too).
	if app.workspaceFlagSet && !app.dirFlagSet {
		dir = ""
	}
	if dir == "" {
		// Workspace-first:
