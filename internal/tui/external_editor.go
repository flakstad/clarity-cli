...
type externalViewEditorDoneMsg struct {
	err error
}
...
func (m *appModel) openExternalEditorForViewBody(body string) (tea.Cmd, error) {
	editor := externalEditorName()
	args := splitShellWords(editor)
	if len(args) == 0 {
		args = []string{"vi"}
	...
	f, err := os.CreateTemp("", "clarity-view-*.md")
	if err != nil {
		return nil, err
	}
	path := f.Name()
	...
	m.externalViewEditorPath = path

	cmd := exec.Command(args[0], append(args[1:], path)...)
	return tea.ExecProcess(cmd, func(err error) tea.Msg {
		return externalViewEditorDoneMsg{err: err}
	}), nil
}
...
func (m *appModel) applyExternalViewEditorResult(msg externalViewEditorDoneMsg) {
	path := m.externalViewEditorPath
	m.externalViewEditorPath = ""
	defer func() { _ = os.Remove(path) }()
	...
	m.showMinibuffer(fmt.Sprintf("Closed %s", externalEditorName()))
}
