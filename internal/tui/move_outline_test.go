...
func TestMoveOutlinePicker_ShowsProjectAndOutlineName(t *testing.T) {
	dir := t.TempDir()
	now := time.Now().UTC()
	...
	db := &store.DB{
		...
		Projects: []model.Project{
			{ID: "proj-a", Name: "Project A", CreatedBy: humanID, CreatedAt: now},
			{ID: "proj-b", Name: "Project B", CreatedBy: humanID, CreatedAt: now},
		},
		Outlines: []model.Outline{
			{ID: "out-a", ProjectID: "proj-a", Name: name("Inbox"), StatusDefs: store.DefaultOutlineStatusDefs(), CreatedBy: humanID, CreatedAt: now},
			{ID: "out-b", ProjectID: "proj-b", Name: name("Inbox"), StatusDefs: store.DefaultOutlineStatusDefs(), CreatedBy: humanID, CreatedAt: now},
		},
		Items: []model.Item{
			{
				ID:           "item-a",
	...
	items := m2.outlinePickList.Items()
	if got := len(items); got != 2 {
		t.Fatalf("expected 2 outline options; got %d", got)
	}
	...
	for _, it := range items {
		...
		if !ok {
			t.Fatalf("expected outlineMoveOptionItem; got %T", it)
		}
		titles = append(titles, oi.Title())
	}

	if titles[0] != "Project A / Inbox" {
		t.Fatalf("unexpected first title: %q", titles[0])
	}
	if titles[1] != "Project B / Inbox" {
