...
func TestValidateCaptureTemplates_Groups(t *testing.T) {
	t.Run("accepts_valid_groups", func(t *testing.T) {
		cfg := &GlobalConfig{
			CaptureTemplates: []CaptureTemplate{
				{
					Name: "Work inbox",
			...
			CaptureTemplateGroups: []CaptureTemplateGroup{
				{Name: "Work", Keys: []string{"w"}},
			},
		}
		if err := ValidateCaptureTemplates(cfg); err != nil {
			t.Fatalf("unexpected error: %v", err)
	...
	t.Run("rejects_duplicate_group_keys", func(t *testing.T) {
		cfg := &GlobalConfig{
			CaptureTemplates: []CaptureTemplate{
				{
					Name: "Work inbox",
	...
	t.Run("rejects_groups_that_match_nothing", func(t *testing.T) {
		cfg := &GlobalConfig{
			CaptureTemplates: []CaptureTemplate{
				{
					Name: "Work inbox",
