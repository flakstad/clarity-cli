...
func (m *appModel) gitStatusBadgeText() string {
        ...
        if m.autoCommit != nil {
                cs := m.autoCommit.Status()
                if cs.Running || cs.Pending {
                        return "[git: syncing]"
                }
                // If the background sync loop recently failed, surface it in the breadcrumb.
                if strings.TrimSpace(cs.LastError) != "" && !cs.LastAt.IsZero() && time.Since(cs.LastAt) < 2*time.Minute {
                        return "[git: sync err]"
                }
        }
        if st.DirtyTracked {
