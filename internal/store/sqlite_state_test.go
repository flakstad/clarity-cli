...
func TestSQLiteStateStore_SaveLoad_RoundTrip(t *testing.T) {
        withEnv(t, envStoreBackend, string(StoreBackendSQLite), func() {
        ...
        withEnv(t, envStoreBackend, string(StoreBackendSQLite), func() {
                withEnv(t, envEventLogBackend, string(EventLogBackendSQLite), func() {
                        withEnv(t, "CLARITY_CONFIG_DIR", t.TempDir(), func() {
                                ...
                                // Create a legacy db.json using JSON backend.
                                withEnv(t, envStoreBackend, string(StoreBackendJSON), func() {
                                        s := Store{Dir: dir}
                                        now := time.Now().UTC()
                                        legacy := &DB{
                                                Version:          1,
                                ...
                                // Now load with SQLite backend; should import.
                                s2 := Store{Dir: dir}
                                got, err := s2.Load()
                                if err != nil {
